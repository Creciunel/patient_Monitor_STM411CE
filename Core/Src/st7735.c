...
void ST7735_fillCircle(int16_t x0, int16_t y0, int16_t r, uint16_t color) {
	...
	ST7735_drawFastHLine(x0 - r, y0, 2 * r + 1, color);
	...
	while (x < y) {
		...
		ST7735_drawFastHLine(x0 - x, y0 + y, 2 * x + 1, color);
		ST7735_drawFastHLine(x0 - x, y0 - y, 2 * x + 1, color);
		ST7735_drawFastHLine(x0 - y, y0 + x, 2 * y + 1, color);
		ST7735_drawFastHLine(x0 - y, y0 - x, 2 * y + 1, color);
	}
}

void drawStethoscope(uint16_t x, uint16_t y, uint16_t width, uint16_t height) {
	// Draw the head of the stethoscope
	ST7735_fillCircle(x + width / 2, y + height / 4, height / 4, ST7735_BLUE);

	// Draw the tubes of the stethoscope
	ST7735_FillRectangle(x + width / 4, y + height / 4, width / 2, height / 2,
	ST7735_BLUE);

	// Draw the ear pieces of the stethoscope
	ST7735_fillCircle(x + width / 4, y + height * 3 / 4, height / 8,
	ST7735_BLUE);
	ST7735_fillCircle(x + width * 3 / 4, y + height * 3 / 4, height / 8,
	ST7735_BLUE);
}

void drawRedCross(void) {
//  ST7735_setRotation(1);  // Set the display rotation, 1 for landscape mode
	ST7735_FillScreen(ST7735_BLACK);  // Clear the screen

	uint16_t centerX = ST7735_WIDTH / 2; // Calculate the center X position of the screen
	uint16_t centerY = ST7735_HEIGHT / 2; // Calculate the center Y position of the screen

	uint16_t crossLength = 20;  // Length of the cross lines
//  uint16_t crossThickness = 2;  // Thickness of the cross lines

	// Draw the horizontal line of the cross
	ST7735_drawFastHLine(centerX - crossLength / 2, centerY, crossLength,
	ST7735_RED);

	// Draw the vertical line of the cross
	ST7735_drawFastVLine(centerX, centerY - crossLength / 2, crossLength,
	ST7735_RED);
}
